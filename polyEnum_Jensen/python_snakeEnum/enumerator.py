# This is the snake enumerator class to enumerate number of snakes of size n.
...
class PolyominoEnumerator:
    def __init__(self, n):
        self.n = n
        self.w_max = (n + 1) // 2 # might be issue for even n's ?
    ...
    def run_for_w(self, w) -> None:
        ...
        initial_signature = Signature([0]*w, [False]*w)
        ...
        for col in range(1, L_max + 1):
            if col == 2:  # Just make sure you flush out the all 0 signature after first row
            ...
            for row in range(w):
                ...
                for sig, gf in table.items():
                    ...
                    # Option 1: It seals a valid snake polyomino
                    if sig.can_add(row):
                        length = col-1
                        if length < w:
                    ...
                    # Option 2: It does not create a valid snake polyomino, update signature table
                    else:
